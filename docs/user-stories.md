...
* **User Story 11 (Visual Summary):** As a user reviewing my trip, I want to see a high-level visual summary of the itinerary, so that I can quickly grasp the overall flow of my travel.
  * **Status:** `[x]` Completed
  * *Notes:* Scenario comparison cards provide an at-a-glance visual summary (price, stress, leg overview), and the scenario detail modal adds a per-direction travel timeline and expandable day-by-day plan. An optional interactive map view is still pending Phase 8.
