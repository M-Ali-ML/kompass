...
* **Interactive Map (Phase 8):** a persistent split-panel ([map/trip-panel.js](file:///Users/aly/repos/kompass/frontend/app/components/map/trip-panel.js)) shows an itinerary summary above an interactive Google Map ([map/trip-map.js](file:///Users/aly/repos/kompass/frontend/app/components/map/trip-map.js), built on `@vis.gl/react-google-maps`). It plots numbered transit stops + accommodation markers and mode-colored polylines (geodesic for flights), auto-fitting bounds. Endpoint coordinates are geocoded client-side ([lib/geocode.js](file:///Users/aly/repos/kompass/frontend/app/lib/geocode.js): Google Geocoder, `localStorage`-cached, IATA→airport disambiguation). The active route flows through a shared `MapState` store ([map/map-context.js](file:///Users/aly/repos/kompass/frontend/app/components/map/map-context.js)) fed by the scenario cards (auto-select best-value + per-card "View on map"). Per-mode glyphs/colors are shared via [lib/transport.js](file:///Users/aly/repos/kompass/frontend/app/lib/transport.js). Requires `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` (browser key); degrades to a placeholder when unset.
* **Implementation Status:** CopilotKit React SDK v2 chat with streamed reasoning/tool events, a two-tab trip/saved sidebar, six generative-UI tool renderers, a full scenario detail modal, and an interactive Google Map split-panel. Leaflet/react-leaflet remain installed as an optional no-key map fallback.
